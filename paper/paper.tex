...
\caption{Per-topic \emph{indirect}-category classification for six frontier-scale fully-measured models (frontier plus non-Maritaca smaller models). Columns: \textsc{Sab4}=Sabi\'a-4, \textsc{Opus}=Opus 4.6, \textsc{GPT5}=GPT-5.4, \textsc{Grok}=Grok 4.20, \textsc{GemP}=Gemini 3.1 Pro, \textsc{Qwn3}=Qwen3.5-397B. Smaller models (Haiku 4.5, GPT-5.4-mini, Gemini 3.1 Flash, Sabiazinho-4) are reported in the appendix. Sabiazinho-4 is reported in the appendix. Cell legend: \clsAgree\,=\,consistently agree, \clsDisagree\,=\,consistently disagree, \clsNeutral\,=\,consistently neutral, \clsSycophant\,=\,sycophant (mirrors the user), \clsInconsistent\,=\,inconsistent (no clean pattern), \clsContrarian\,=\,contrarian (pushes back on the user). Detailed per-persona verdicts and the direct-probing analogue are in the appendix.}
